...
import { GetGuacamole } from "./guacamole.module";
...
function InitalizeGuacamoleClient() {
	...
	tunnel.onstatechange = function(state) {
		if (state == Guacamole.Tunnel.State.CLOSED) {
            ...
            // extend/upgrade hack
			function setup(upgraded) {
				delete tunnel.onupgrade;

				extendedSetup(tunnel);
			...
			const t = setTimeout(() => {
				console.debug("upgrade timed out, must be a legacy server");
				setup(false);
			}, 500);
			tunnel.onupgrade = proto => {
				console.debug("upgraded to " + proto);
				clearTimeout(t);
				setup(true);
			}
	...
	guac.onrename = function(parameters) {
		if (parameters[0] === "0") {
			...
			setCookie("nickname", username, 365);
...
$(function() {
	...
	$("#nsfw-cont-btn").click(function() {
		...
		if ($("#no-warn-chkbox").prop("checked")) {
			setCookie("skip_warning", true, 365);
		}
	});
	...
	$("#chat-sound-btn").click(function() {
		...
		setCookie("chatsound", chatSoundOn, 365);
	});

	/*$("#codec-btn").click(() => {
		msgpackLast = !msgpackLast;
		tunnel.sendMessage("upgrade", msgpackLast ? "lec" : "guac");
		$("#codec-btn")[0].innerText = msgpackLast ? "Guac" : "MsgPack";
	});*/
	
	initSound();
	
	setChatSoundOn(getCookie("chatsound"));

	displayNsfwWarn(!common.DEBUG_NO_NSFW && !getCookie("skip_warning"));
