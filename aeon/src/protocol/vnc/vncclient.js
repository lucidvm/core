const { ImageData, Canvas } = require("canvas");
...
class VncClient extends Events {
    ...
    async _handleServerInit() {
        ...
        this.pixelFormat.redMax = this.pixelFormat.bigEndianFlag ? this._socketBuffer.readUInt16BE() : this._socketBuffer.readUInt16LE();
        this.pixelFormat.greenMax = this.pixelFormat.bigEndianFlag ? this._socketBuffer.readUInt16BE() : this._socketBuffer.readUInt16LE();
        this.pixelFormat.blueMax = this.pixelFormat.bigEndianFlag ? this._socketBuffer.readUInt16BE() : this._socketBuffer.readUInt16LE();
     ...
     */
    updateFbSize() {
        this.canvas = new Canvas(this.clientWidth, this.clientHeight);
        this.canvasdraw = this.canvas.getContext("2d");
        this.imgdata = new ImageData(this.clientWidth, this.clientHeight);
        this.fb = Buffer.from(this.imgdata.data.buffer);
    }
    ...
    async _handleRect() {
        ...
        while (this._rects) {
            ...
            if (rect.encoding === encodings.pseudoCursor) {
                ...
                this.emit('cursorChanged', this._cursor);
        ...
        if (sendFbUpdate) {
            for (const rect of rectinfo) {
                this.canvasdraw.putImageData(this.imgdata, 0, 0, rect.x, rect.y, rect.width, rect.height);
            }
            ...
            this.emit('frameUpdated', this.fb, rectinfo);
     ...
     * @param buttonMask - button bitmask
     */
